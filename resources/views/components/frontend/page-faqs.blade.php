...
@php
  $items = collect(is_array($items) ? $items : [])
    ->filter(fn ($i) => filled($i['question'] ?? null));
@endphp

@if ($items->isNotEmpty())
<section class="py-14 bg-offwhite" id="faq" aria-labelledby="page-faq-title">
  <div class="w-full max-w-[1200px] mx-auto px-4 md:px-6 xl:px-8">
    <div class="mb-8">
      <div class="font-display text-xs font-semibold tracking-[.14em] uppercase text-orange">
        <i class="bi bi-question-circle" aria-hidden="true"></i> FAQ
      </div>
      <h2 class="font-display font-bold leading-tight text-navy mt-2 text-[clamp(1.4rem,3.5vw,2rem)]" id="page-faq-title">
        {{ $heading }}
      </h2>
    </div>
